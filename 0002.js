  ...
  .map(r => {
    return r.reduce(
      (acc, curr) => {
        ...
        return {
          high: current > acc.high ? current : acc.high,
          low: current < acc.low ? current : acc.low
        };
      },
      { high: 0, low: Infinity }
    );
...
const partTwo = data.map(r => {
  return r
    .map(s => parseInt(s, 10))
    .sort((a, b) => a - b)
    .reverse()
    .reduce((acc, curr, i, array) => {
      console.log(array.splice(i + 1, array.length));
      [...array.splice(i, array.length)].filter(a => curr % a === 0);
      return [...acc, curr];
    }, []);
});
